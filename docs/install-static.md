...
> [!NOTE]
> These files are generated from the Helm chart by `make docs`. To regenerate
...
> [!TIP]
> If you already run the CCM and want to reuse its credentials edit the
> `xo-cloud-config` volume in the controller Deployment to point at your
> existing secret name (e.g., `xenorchestra-cloud-controller-manager`).
