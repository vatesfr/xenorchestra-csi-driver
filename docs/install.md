...
| `--node-name` | Name of the Kubernetes node this driver instance runs on. Required in both modes — the driver resolves the node's pool metadata from it via the XenOrchestra CCM. | _(none, required)_ |
...
| `--kubernetes-pool-tag` | Tag added to Xen Orchestra pools that are eligible for automatic volume placement when no `poolId` or topology constraints are provided. | `k8s-pool` |
