...
3. **Create a PersistentVolume** referencing the VDI UUID:
   ...
   spec:
     ...
     csi:
       driver: csi.xenorchestra.vates.tech
       volumeHandle: "<xo-vdi-uuid>"  # VDI UUID
   ```
...
> [!TIP]
> The pod using the PVC does not need to be stopped. It will be migrated live.
...
kubectl annotate pvc example-vac-pvc \
  volumeattributesclass.storage.k8s.io=migrate-to-slow-sr
```
