...
type DriverOptions struct {
	...
	// VDIs in ListVolumes so only K8s-managed volumes are returned. Use a unique
	// value per cluster when running multiple clusters against the same XO instance.
	// Defaults to DefaultClusterTag ("k8s-managed"). Set to "" to disable tagging
...
func (o *DriverOptions) AddFlags() *flag.FlagSet {
	...
	fs.StringVar(&o.ClusterTag, "cluster-tag", DefaultClusterTag,
		"Tag added to all VDIs created by this driver and used to filter volumes in ListVolumes. "+
